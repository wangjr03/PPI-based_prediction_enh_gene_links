...
4. Protein-protein interaction (PPI) datasets: PPI datasets can be downloaded from STRING database. To remove the low-quality PPIs, we only use the PPIs with a confidence score greater than 100 in the ‘Experiments’ category for feature generation. This threshold removes around 50% of the PPIs. For TF module detection, the threshold of the PPI confidence score is set to 500. A filtered PPI dataset based on STRING V11 has been integrated into the program.
## Pre-calculated data
