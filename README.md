...
8. Reformat_feature_shuffle.py: This script is used to generate the null probability distribution for potential enhancer-promoter interactions. In this step, each feature is permuted randomly across all samples. The permuted features are then used as the input of the trained random forest model to generate the null probabilities. The input and usage are the same as step 7. 

9. pFDR_procedure.R: This script uses the pFDR procedure to integrate the genomic distance information. The first step is to calculate a p-value for each enhancer-promoter interaction based on the predictive probability from step 7 and null predictive probabilities step 8. The second step is to calculate a q-value using the pFDR procedure based on the genomic distances and p-values.<br>
	**Inputs**: the result of step 7 and step 8.<br>
